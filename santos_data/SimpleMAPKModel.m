classdef SimpleMAPKModel
    ...
    methods(Static)
        ...
        function [dydt1] = dydt(t,y,O)
            ...
            EGF=(O.EGF/(1+O.EGF))*(t^5)*exp(-kd_egf*t);%+(O.EGF^0.333)*(1-exp(-kd_egf*t));
            NGF=(O.NGF/(1+O.NGF))*(t^5)*exp(-kd_ngf*t);%+(O.NGF^0.333)*(1-exp(-kd_ngf*t));
...
%             dydt1(3)=O.MMa(kf31,Kmf31,iEK,aMK) + bn*O.MMa(kf32,Kmf32,iEK,aRF) - O.MM0(Vm3,Km3,aEK);
%             dydt1(1)=be*(O.MMa(kf11,Kmf11,iRF,EGF)*(Kmf12/(Kmf12+aEK))) + bn*(O.MMa(kf13,Kmf13,iRF,NGF)*(1+aEK/(Kmf14+aEK)))- O.MM0(Vm1,Km1,aRF);
%             dydt1(2)=O.MMa(kf21,Kmf21,iMK,aRF)*(Kmf22/(Kmf22+aEK))-O.MM0(Vm2,Km2,aMK);
%             dydt1(3)=O.MMa(kf31,Kmf31,iEK,aMK) + bn*O.MMa(kf32,Kmf32,iEK,aRF) - O.MM0(Vm3,Km3,aEK);
...
%             dydt1=dydt1(:);
            ...
            J=zeros(3);
%             J(1,1)=-O.dMM0_dsub(Vm1,Km1,aRF);
%             J(1,3)=-be*O.MMa(kf11,Kmf11,iRF,O.EGF)*(Kmf12/(Kmf12+aEK)^2)+bn*O.dMMa_dmod(kf14,Kmf14,iRF,aEK);
%             J(2,1)=O.dMMa_dmod(kf21,Kmf21,iMK,aRF)*(Kmf22/(Kmf22+aEK));
%             J(2,2)=-O.dMM0_dsub(Vm2,Km2,aMK);
%             J(2,3)=-O.MMa(kf21,Kmf21,iMK,aRF)*(Kmf22/(Kmf22+aEK)^2);
...
%             
        ...
        function [J,y]=Jacobian_custom_numerical(O,y)
            J=[];
            d=0.0001;
            t=1;
            for i=1:length(y)
         ...
         function [Oi]=simulate_observed_metric(M,params)
             %M=O;
             M.params=[params 1 0];% Stimulate with EGF
             
                %[EGFt,NGFt]=getGrowthFactorLevels(M);
                y=simulate_model(M);
                if size(y,2)==length(M.timespan)
                    y1=y(:,2);   % aRF, aMK, aEK levels at 5 mins
%                     EGFt1=EGFt(3); % EGF levels at 5 mins
%                     NGFt1=NGFt(3); % NGF levels at 5 mins
                    [ri,yi]=M.r_from_Jacobian_custom(M,y1);ri=ri(:);ri1=ri([2:4 6:8]); % Calculate local response coefficients
                    dydt1=M.dydt(5,y1,M);
                    
                end
                M.params=[params 0 1];% Stimulate cells with NGF
                y_1=simulate_model(M);
                if size(y_1,2)==length(M.timespan)
                    y1=y_1(:,4);   % aRF, aMK, aEK levels at 15 mins                    
                    [ri,yi]=M.r_from_Jacobian_custom(M,y1);ri=ri(:);ri2=ri([2:4 6:8]); % Calculate local response coefficients
                    dydt2=M.dydt(15,y1,M); % Calculate rates
                end
                
                if size(y,2)==length(M.timespan) && size(y_1,2)==length(M.timespan)
                    Oi=[ri1' y(3,:)/max(y(3,:))  dydt1' ri2' y_1(3,:)/max(y_1(3,:)) dydt2'];
...
%                 y=simulate_model(M);
% %                 if size(y,2)==length(M.timespan)
% %                     y1=y(:,3);   % aRF, aMK, aEK levels at 5 mins
% % %                     EGFt1=EGFt(3); % EGF levels at 5 mins
% % %                     NGFt1=NGFt(3); % NGF levels at 5 mins
% %                     [ri,yi]=M.r_from_Jacobian_custom(M,y1);ri=ri(:);ri1=ri([2:4 6:8]); % Calculate local response coefficients
% %                     dydt1=M.dydt(5,y1,M);
...
%                 y_1=simulate_model(M);
% %                 if size(y_1,2)==length(M.timespan)
% %                     y1=y_1(:,6);   % aRF, aMK, aEK levels at 15 mins                    
% %                     [ri,yi]=M.r_from_Jacobian_custom(M,y1);ri=ri(:);ri2=ri([2:4 6:8]); % Calculate local response coefficients
% %                     dydt2=M.dydt(15,y1,M); % Calculate rates
% %                 end
% %                 
%                 if size(y,2)==length(M.timespan) && size(y_1,2)==length(M.timespan)
%                     Oi=[y(1,:)/max(y(1,:)) y(2,:)/max(y(2,:)) y(3,:)/max(y(3,:))   y_1(1,:)/max(y_1(1,:)) y_1(2,:)/max(y_1(2,:)) y_1(3,:)/max(y_1(3,:))];
%                     
%                 else
%                     Oi=0;
%                 end
