classdef ABC_SMC_WEST
    ...
    properties
        %e=[75,50,40,30,25,20,15,13, 11.5, 10:-1:3 2.5,2:-0.25:0.75,0.6,0.5,0.4,0.35,0.3,0.25:-0.05:0]; %Threshold schedule
        e=[85:-1:20 19.5:-0.5:10 9.75:-0.25:2 1.9:-0.1:1 0.95:-0.05:0.7 0.675:-0.025:0]
        N=100;% lower bound on the number of samples in each population, 
        Nq=16;% Dimention of the paramteres;
        Nb=60;%Blocks of parameters that will be evaluated in parallel
        Nd=45;% Dimension of the data
        prior_params=struct('mu',log(12.5)*ones(1,16),'sig',1*ones(1,16),'Indexes',1:16);%prior distribution of the parameter
        %sigma=1;% Noise level;
         ...
         function [Q_s,d,interrupted]=find_N_params_within_e_ball_from_prev_samples(Qs,W,D_obs,D_params,e,O)
             ...
             while count<O.N % Until all N, has been sampled
                 ...
                 if count+count1<=O.N % the last samples do not fullfill the total requirement
                     ...
                     if total_count>2000 && acc_rate<=0.1
                       interrupted=true;
                       break;
                     end
                 else % if more than required samples are within e ball
